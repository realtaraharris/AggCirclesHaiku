...
// as follows:
//
//  class Circles : public PlatformSupport
//  {
//  public:
...
public:
    ...
    agg::rendering_buffer& rbuf_img(unsigned idx) { return m_rbuf_img[idx]; }
    ...
    void add_ctrl(agg::ctrl& c) { m_ctrls.add(c); c.transform(m_resize_mtx); }
    ...
    void trans_affine_resizing(int width, int height) {
        if(m_window_flags & window_keep_aspect_ratio) {
            ...
            m_resize_mtx = agg::trans_affine_scaling(
                double(width) / double(m_initial_width),
                double(height) / double(m_initial_height));
        }
    }
    const    agg::trans_affine& trans_affine_resizing() const { return m_resize_mtx; }
...
private:
    ...
    agg::rendering_buffer m_rbuf_img[max_images];
